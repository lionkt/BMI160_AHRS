...
u8 File_SecBuf[512];	//二级缓冲区，用来存放向SD卡写入的数据
u8 File_Buf[4096];		//一级缓冲区用来存放TIM4中断读到的信息，但每次只读取File_SecBuf的内容
uint16_t FBindex = 0;
...
{
	int16_t i;
	for (i = 0; i < 512; i++)	//从buf读出来512bytes
	{
		data[i] = File_Buf[i];
	}
	for (i = 0; i < FBindex - 512; i++)		//将超过512 bytes的内容前移
	{
