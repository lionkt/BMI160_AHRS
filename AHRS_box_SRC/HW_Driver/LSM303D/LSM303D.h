...
//LSM303 片选信号控制
#define LSM303_CSH()  GPIO_SetBits(GPIOB, GPIO_Pin_0)
#define LSM303_CSL()  GPIO_ResetBits(GPIOB, GPIO_Pin_0)

extern int16_t  lastAx,lastAy,lastAz,  //最新的加速度ADC值
		 		lastMx,lastMy,lastMz;  //最新的磁力计ADC值

extern int16_t  Mag_maxx,Mag_maxy,Mag_maxz,	//当前磁场的最大值和最小值
		 Mag_minx,Mag_miny,Mag_minz;
extern unsigned char Mag_calib; //磁标定，完成标志

void LSM303_Initial(void);	//初始化LSM303
u8 LSM303_readID(void);	 //读ID 正确返回0x49
void LSM303_readAcc(int16_t *acc);	//读加速度ADC数据
void LSM303_readMag(int16_t *Mag);	//读磁力计ADC数据
void LSM303_Start_Calib(void);	 //启动磁标定
void LSM303_Save_Calib(void);	 //完成磁标定，保存标定信息
void LSM303_update_config(void); //加载标定数据
