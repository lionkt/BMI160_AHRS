...
#define FILE_BUF_LENGTH 100 //crown add，定义文件缓冲区的长度
...
void File_Save_Routing(void)
{
	...
	if (Data_Ready)
	{
		...
		if (now_toBuf_ptr <= toFile_ptr)
		{
			...
			if (mf_write(File_Data_buf + start_index, sizeof(File_Data) * (FILE_BUF_LENGTH - toFile_ptr)) == 0)
				;				   //先写一段
			if (now_toBuf_ptr > 0) //如果now_toBuf_ptr前面还有数据，则再写另一段
			{
				if (mf_write(File_Data_buf, sizeof(File_Data) * (now_toBuf_ptr)) == 0)
					;
			}
		}
		else
		{
			...
			if (mf_write(File_Data_buf + start_index, sizeof(File_Data) * temp_write_length) == 0)
				; //这种情况下可以一次写完
		}
