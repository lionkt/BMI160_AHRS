...
int main(void)
{
	...
	FileSave_TimerSet(200); //设置保存数据的频率，crown change to 200hz
  ...
  while (1)
  {
	...
	case Gyro_init:		BMI160_InitGyro_Offset(); break; //读取陀螺仪零偏
    case HMC_calib:		QMC5883L_Save_Calib();	
						break;   //保存磁力计标定
	case High_init:			MS561101BA_ResetAlt();	break;		//气压高度 清零
	case HMC_calib_begin:	QMC5883L_Start_Calib();	break; //开始磁力计标定
	case 0xA0: ACC_Cal(rx_buffer[2]); break;
	case 0xE4: ACC_Save_cal(); break;
	case 0xE6: BMI160_Reset_ACC_Offset(); break;
	}
	}// 处理PC 发送的命令
...
//------------电源处理-----------		  
	while(Check_POW()){	  //从机要求关机
					mf_close();
					while(1)
		  		LED_OFF(); //检测到关机信号 PB9 低电平
		  }
		  if(Is_BAT_LOW()){	  //电池电压低于3.6V 提示关机
		   		LED_OFF();
					SET_POWER_Down();
					mf_close();
					while(1); 	   
...
void USB_Check(void){
	...
	if((USB_Staut == USB_OTG_DEFAULT)||(USB_Staut == USB_OTG_SUSPENDED))return;
	BEEP_ON();
	delay_ms(100);
	BEEP_OFF();
	while(1){
		  if(Divece_STA == 0x01){ //正在读写SD卡
			  LED_OFF();
			  delay_ms(50);
			  Divece_STA = 0;
		  }
